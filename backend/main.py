...
from fastapi import FastAPI, UploadFile, File, Form, HTTPException, Body, Depends, status, Request, WebSocket, WebSocketDisconnect
...
from auth import decode_access_token  # forward import for the key fn
...
# ─── EMAIL ──────────────────────────────────────────────────
from email_service import send_otp_email, send_resend_otp_email
...
def _analyze_rate_key(request: Request) -> str:
    """
    Rate-limit key for /analyze:
    - Authenticated users: keyed by user_id  -> 5 scans/user/day
    - Anonymous requests:  keyed by IP       -> 5 scans/IP/day
    """
...
# Use IP for auth endpoints, user-aware key for analyze
limiter = Limiter(key_func=get_remote_address)
app.state.limiter = limiter

@app.exception_handler(RateLimitExceeded)
async def _rate_limit_handler(request: Request, exc: RateLimitExceeded):
    """Return a clean 429 JSON instead of the default HTML page."""
    # Parse the limit string from the exception (e.g. "5 per 1 day")
    limit_str = str(exc.detail) if hasattr(exc, 'detail') else str(exc)
    return JSONResponse(
        status_code=429,
        content={
            "detail": f"Rate limit exceeded: {limit_str}. Please try again later.",
...
def ping_server():
    ...
    try:
        # Priority: RENDER_EXTERNAL_URL (Render) > SPACE_HOST (HF Spaces) > localhost
        render_url = os.environ.get("RENDER_EXTERNAL_URL")
        space_host = os.environ.get("SPACE_HOST")  # HF Spaces provides this automatically
...
async def startup_event():
    ...
    print(f"[FairAI] Starting parallel worker pool with {MAX_CONCURRENT_WORKERS} workers...")
...
def auth_register(request: Request, body: RegisterRequest):
    """
    Register a new user.
    1. Creates the user row (is_verified=False)
    2. Generates a 6-digit OTP + emails it
    3. Returns {pending_verification: True} — NO token yet.
    The client must call /auth/verify-otp to get an access token.
    """
    ...
    if existing:
        if existing.is_verified:
            ...
            # Account exists but not verified — resend OTP instead of erroring
    ...
    user = create_user(
        ...
        role=body.role if body.role in ("recruiter", "candidate") else "recruiter",
        org=body.org,
    )
    if user is None:
    ...
    if not sent:
        # Email send failed — still allow flow, OTP is in server logs for dev
        print(f"[Auth] WARNING: OTP email failed to send for {email!r}")
...
def auth_verify_otp(body: VerifyOTPRequest):
    """
    Verify the 6-digit OTP. On success:
      - Marks user as verified
      - Returns access_token + user object (same as login response)
    On failure: 400 with explanation.
    """
    email = body.email.lower().strip()
    otp   = body.otp.strip()
...
@limiter.limit("5/hour")   # max 5 OTP resend requests per IP per hour
def auth_resend_otp(request: Request, body: LoginRequest):
    """
    Resend a fresh OTP to an email address.
    body.password is ignored — only email is used.
    Rate-limit note: create_otp() invalidates the previous code before issuing a new one.
    """
    email = body.email.lower().strip()
    ...
    otp = create_otp(email)
    if otp:
        send_resend_otp_email(email, otp, user.name)

    return {"message": "New verification code sent. Check your inbox."}


@app.post("/auth/login")
@limiter.limit("3/minute")   # brute-force protection: 3 attempts per IP per minute
def auth_login(request: Request, body: LoginRequest):
    """Authenticate user with email + password. Returns access token."""
...
# ─── USER SCAN HISTORY ENDPOINTS ─────────────────────────────

@app.get("/user/scans")
def get_user_scan_history(current_user=Depends(get_current_user)):
    """Return the most recent 20 scan records for the authenticated user."""
    scans = get_user_scans(current_user.id, limit=20)
    return {"scans": scans, "total": len(scans)}


@app.get("/user/scans/bias-heatmap")
def get_bias_heatmap(current_user=Depends(get_current_user)):
    """
    Returns aggregate bias signal counts per Kanban stage for the authenticated user.
    Powers the column-level Bias Heatmap in the frontend Kanban board.
    IMPORTANT: Must be registered BEFORE /user/scans/{scan_id}/result to avoid
    FastAPI trying to parse 'bias-heatmap' as an integer scan_id.
    """
    try:
        with SessionLocal() as db:
            scans = db.query(ScanRecord).filter(
                ScanRecord.user_id == current_user.id,
                ScanRecord.result_json.isnot(None)
            ).all()

            VALID_STAGES = ["Sourced", "Screening", "Interview", "Offer", "Rejected"]
            heatmap = {
                stage: {"total": 0, "bias_count": 0, "high_count": 0, "bias_types": set()}
                for stage in VALID_STAGES
            }

            for scan in scans:
                stage = scan.kanban_stage or "Sourced"
                if stage not in heatmap:
                    continue
                heatmap[stage]["total"] += 1
                try:
                    result = json.loads(scan.result_json) if isinstance(scan.result_json, str) else scan.result_json
                    proxies = result.get("bias_proxies") or []
                    if proxies:
                        heatmap[stage]["bias_count"] += len(proxies)
                        for p in proxies:
                            severity = p.get("severity", "low")
                            if severity == "high":
                                heatmap[stage]["high_count"] += 1
                            bias_type = p.get("bias_type")
                            if bias_type:
                                heatmap[stage]["bias_types"].add(bias_type)
            ...
            for stage, data in heatmap.items():
                ...
                total_bias = data["bias_count"]
                density = (
                    "critical" if (stage == "Rejected" and high >= 2)
                    else "high" if (high >= 3 or total_bias >= 4)
                    else "medium" if (high >= 1 or total_bias >= 2)
                    else "none"
                )
                result_heatmap[stage] = {
                    "total": data["total"],
                    "bias_count": total_bias,
                    "high_count": high,
                    "bias_types": list(data["bias_types"]),
                    "density": density,
                }
        ...
        print(f"[BiasHeatmap] Error: {e}", file=sys.stderr)
        raise HTTPException(status_code=500, detail="Failed to compute bias heatmap")
...
def get_scan_result(
    ...
    """Fetch the full result_json for a specific scan (lazy-loaded for Kanban drawer)."""
    try:
        with SessionLocal() as db:
            ...
            if not scan.result_json:
                raise HTTPException(status_code=404, detail="Result not available for this scan")
            ...
            return {
                "scan_id": scan.id,
                "role_target": scan.role_target,
                "fit_score": scan.fit_score,
        ...
        print(f"[FairAI] get_scan_result failed: {e}", file=sys.stderr)
        raise HTTPException(status_code=500, detail="Failed to fetch scan result")


@app.patch("/user/scans/{scan_id}/stage")
def update_scan_kanban_stage(
    scan_id: int,
    body: KanbanStageUpdate,
    current_user=Depends(get_current_user),
):
    """Move a candidate card to a different Kanban pipeline stage."""
    VALID = {"Sourced", "Screening", "Interview", "Offer", "Rejected"}
    if body.stage not in VALID:
        raise HTTPException(status_code=422, detail=f"Invalid stage. Must be one of: {', '.join(VALID)}")
    ok = update_kanban_stage(scan_id=scan_id, user_id=current_user.id, stage=body.stage)
    if not ok:
        raise HTTPException(status_code=404, detail="Scan record not found or access denied.")
    
    # Return the updated scan record
    with SessionLocal() as db:
        scan = db.query(ScanRecord).filter(
            ScanRecord.id == scan_id,
            ScanRecord.user_id == current_user.id
        ...
        # Safely get stage_updated_at if it exists (may not be in all DB schemas)
        stage_updated_at = getattr(scan, 'stage_updated_at', None)
        if stage_updated_at:
            if isinstance(stage_updated_at, str):
                stage_updated_at_str = stage_updated_at
            else:
                stage_updated_at_str = stage_updated_at.isoformat()
        else:
            stage_updated_at_str = None
        ...
        return {
            ...
            "kanban_stage": scan.kanban_stage,
            "stage_updated_at": stage_updated_at_str,
            "recruiter_notes": getattr(scan, 'recruiter_notes', None),
            "batch_id": scan.batch_id,
            "has_result": bool(scan.result_json),
            "result_json": scan.result_json
        }

@app.patch("/user/scans/{scan_id}/notes")
def update_scan_recruiter_notes(
    scan_id: int,
    body: ScanNotesUpdate,
    current_user=Depends(get_current_user),
):
    """Save private recruiter notes for a candidate card."""
...
@app.post("/user/scans/{scan_id}/interview-guide")
async def generate_interview_guide(
    scan_id: int,
    current_user=Depends(get_current_user),
):
    """
    Generate a tailored 5-question interview guide for a candidate using their
    skill strengths and gaps from result_json. Saves the guide as recruiter_notes.
    """
    if not client:
    ...
    try:
        ...
        if not scan:
            raise HTTPException(status_code=404, detail="Scan not found")
        if not scan.result_json:
            raise HTTPException(status_code=400, detail="No analysis result found. Run /analyze first.")

        result = json.loads(scan.result_json) if isinstance(scan.result_json, str) else scan.result_json

        role = scan.role_target or "the target role"
        skill_matches = result.get("skill_matches") or result.get("skills_matched") or []
        gaps = result.get("gaps") or []
        strengths = result.get("strengths") or []
        fit_score = scan.fit_score or 0

        # Build human-readable skill summaries for the prompt
        top_skills = ", ".join([
            s.get("canonical_name") or s.get("skill") or "Unknown"
        ...
        elif strengths and isinstance(strengths[0], dict):
            strength_list = ", ".join([s.get("text") or str(s) for s in strengths[:3]])
        else:
            strength_list = "not specified"

        prompt = f"""You are a senior technical interviewer helping a recruiter prepare for a candidate interview.

Candidate Profile:
- Role: {role}
- Overall Fit Score: {fit_score}/100
- Top Skills Demonstrated: {top_skills}
- Key Strengths: {strength_list}
- Skill Gaps: {gap_list}

Generate exactly 5 targeted interview questions for this specific candidate. The questions must:
1. Probe DEPTH on their strongest skills (not just surface-level definitions)
2. Gently surface their skill gaps without being confrontational
3. Be practical and scenario-based, not theoretical
4. Be ready to use immediately — no filler, no preamble

Return ONLY a JSON object in this exact format, nothing else:
{{"questions": [
  {{"number": 1, "area": "<skill or gap being probed>", "question": "<the full interview question>"}},
  {{"number": 2, "area": "<skill or gap being probed>", "question": "<the full interview question>"}},
  {{"number": 3, "area": "<skill or gap being probed>", "question": "<the full interview question>"}},
  {{"number": 4, "area": "<skill or gap being probed>", "question": "<the full interview question>"}},
  {{"number": 5, "area": "<skill or gap being probed>", "question": "<the full interview question>"}}
]}}"""
        ...
        # Persist as recruiter_notes so it survives refresh
        notes_text = "[INTERVIEW GUIDE]\n" + "\n".join([
            f"Q{q['number']} [{q['area']}]: {q['question']}"
            for q in questions
        ])
        scan.recruiter_notes = notes_text
        db.commit()

        print(f"[InterviewGuide] Generated guide for scan_id={scan_id} by user_id={current_user.id}")
        return {"scan_id": scan_id, "guide": questions, "saved_as_notes": True}
        ...
        print(f"[InterviewGuide] Error for scan_id={scan_id}: {e}", file=sys.stderr)
...
# WebSocket connections manager: batch_id -> set of WebSocket connections
batch_connections = {}
...
async def scan_worker_loop():
    """Parallel worker pool that maintains up to MAX_CONCURRENT_WORKERS active tasks."""
    global active_tasks
    while True:
        try:
            # Keep pool filled with up to MAX_CONCURRENT_WORKERS tasks
            while len(active_tasks) < MAX_CONCURRENT_WORKERS:
                try:
                    ...
                    print(f"[Worker Pool] Started job {job_id} (pool size: {len(active_tasks)}/{MAX_CONCURRENT_WORKERS})")
            ...
            for task in done:
                try:
                    ...
                    print(f"[Worker Pool] Task failed: {e}")
                    
        except Exception as e:
            print(f"[Worker Error] {e}")
            await asyncio.sleep(0.1)

async def process_job_with_stages(job_id, kwargs, batch_id):
    """Process a job through all pipeline stages, emitting WebSocket updates."""
    try:
        ...
        # Execute the full job (pass batch_id to execute_scan_job)
...
@limiter.limit("2/hour", key_func=_analyze_rate_key)  # batch is expensive — 2 batches/hour
async def batch_analyze_resumes(
    request: Request,
    files: list[UploadFile] = File(...),
    role: str = Form(default="Professional"),
    jd_skills: str = Form(default=""),
    current_user=Depends(get_current_user),   # must be authenticated
):
    """
    Queue up to 20 resumes for batch processing.
    Each file gets its own job_id so the frontend can poll each independently.
    Returns immediately with [{filename, job_id, status}] — no waiting.

    Requires authentication. In future this will gate on plan_tier='pro'.
    """
    global scan_queue
    if scan_queue is None:
        raise HTTPException(status_code=503, detail="Queue not initialized")

    if current_user.role == "recruiter" and not current_user.is_recruiter_verified:
        raise HTTPException(status_code=403, detail="Company email verification is required to use Batch Processing.")
    ...
    # Generate a unique batch_id to group all jobs in this batch
    ...
    for f in files:
        ...
        job_store[job_id] = {
            ...
            "result": None,
            "error": None,
            "filename": f.filename,  # Store filename so frontend can display it
            "batch_id": batch_id,
            "stage": 0,
            "stage_name": "Extracting Text",
        }
        ...
        queued.append({"filename": f.filename, "job_id": job_id, "status": "queued"})

    print(f"[Batch] Queued {len(queued)} jobs for user {current_user.id} — batch_id: {batch_id} — role: {role}")
    ...
    return {
        ...
        "jobs": queued,
        "skipped": skipped,
        "message": f"{len(queued)} resumes queued for processing.",
    }
...
async def websocket_batch_updates(websocket: WebSocket, batch_id: str):
    ...
    Clients connect with: ws://api.aethel.com/ws/batch/{batch_id}
    ...
    # Add this connection to the batch's connection set
    if batch_id not in batch_connections:
        batch_connections[batch_id] = set()
...
async def execute_scan_job(job_id: str, filename: str, ext: str, pdf_bytes: bytes, role: str, jd_skills: str, current_user_id: int, batch_id: str = None, stage_callback=None):
    try:
