    ...
    if modal_url:
        try:
            ...
            if data.get("structured_data"):
                ...
                print(f"[Bot3] Modal output keys: {list(result.keys())}")
                print(f"[Bot3] Modal technical_skills: {result.get('technical_skills', [])[:3]}...")
                print(f"[Bot3] Modal job_history: {len(result.get('job_history', []))} entries")
                print(f"[Bot3] Modal highest_degree: {result.get('highest_degree', 'N/A')}")
                return result
            elif data.get("error"):
...
_DEGREE_PATTERNS = [
    ...
    ("Master",      re.compile(r"\b(master(?:'?s)?|m\.s\.|m\.a\.|m\.tech|m\.e\.|mba|m\.eng|mca)\b", re.I)),
    ("Bachelor",    re.compile(r"\b(bachelor(?:'?s)?|b\.?s\.?|b\.?a\.?|b\.?e\.?|b\.?eng|b\.?tech|b\.?comm|b\.?sc|bca)\b", re.I)),
