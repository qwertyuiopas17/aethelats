...
class ScanRecord(Base):
    ...
    recruiter_notes = Column(Text, nullable=True)       # Private notes per candidate (max 2000 chars)
    stage_updated_at = Column(DateTime(timezone=True), nullable=True)  # Timestamp of last stage change
...
def init_db() -> None:
    ...
    try:
        ...
        _run_migration("scan_records", "stage_updated_at", "DATETIME")
...
def update_kanban_stage(scan_id: int, user_id: int, stage: str) -> bool:
    ...
    Sets stage_updated_at to current UTC time when stage changes.
...
def update_scan_notes(scan_id: int, user_id: int, notes: str) -> bool:
    ...
    Args:
        ...
        notes: Note text (max 2000 chars, enforced at API layer)
    """
    try:
        with SessionLocal() as db:
            record = db.query(ScanRecord).filter(
            ...
            if not record:
                return False
            record.recruiter_notes = notes[:2000]  # Defensive truncation
            db.commit()
            print(f"[Kanban] scan_id={scan_id} notes updated by user_id={user_id}")
            return True
    except Exception as e:
