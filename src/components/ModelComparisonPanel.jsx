...
/* ─── Colour helpers ─────────────────────────────────────────── */
const deltaColor  = (d) => Math.abs(d) > 8 ? 'text-red-400' : Math.abs(d) > 3 ? 'text-yellow-400' : 'text-white/50';
...
function RadarDiff({ fairai, other, label }) {
  ...
  return (
    <div className="flex items-center gap-2 text-xs">
      <span className="w-28 shrink-0 text-white/40 truncate">{label}</span>
      <div className="flex-1">
        <MiniBar value={fairai} color="bg-white/40" />
      </div>
      <span className="w-8 text-right font-bold text-white/60">{fairai}</span>
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4">
      ...
      <div className={`rounded-xl border p-3 ${accent}`}>
        ...
        {signals.length === 0 ? (
          <p className="text-xs text-white/20 italic">None detected</p>
        ) : (
          <div className="space-y-2">
            {signals.map((sig, i) => (
              <div key={i} className="flex items-start gap-2">
                <div className={`w-1.5 h-1.5 rounded-full mt-1 shrink-0 ${isOwn ? 'bg-emerald-400/60' : 'bg-white/30'}`} />
                <div>
                  <div className="text-xs font-semibold text-white/60">{sig.signal}</div>
                  {sig.evidence && sig.evidence !== sig.signal && (
                    <div className="text-[10px] text-white/25 italic mt-0.5">"{sig.evidence}"</div>
                  )}
                </div>
              </div>
            ))}
      ...
      <div className={`rounded-xl border p-3 ${accent}`}>
        <div className="flex items-center gap-1.5 mb-2">
          <TrendingDown className="w-3.5 h-3.5 text-white/20" />
          <span className="text-xs font-bold text-white/30 uppercase tracking-wider">
            Gaps ({gaps.length})
          </span>
        </div>
        {gaps.length === 0 ? (
          <p className="text-xs text-white/20 italic">No gaps flagged</p>
        ) : (
          <div className="space-y-2">
            {gaps.map((g, i) => (
              <div key={i} className="flex items-start gap-2">
                ...
                <div>
                  <div className="text-xs font-semibold text-white/50">{g.gap}</div>
    ...
    <div className={`rounded-2xl border transition-all duration-200 overflow-hidden ${isOwnModel
      ...
      <div className="px-5 py-4 flex items-center gap-4">
        <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 border ${isOwnModel ? 'bg-emerald-400/10 border-emerald-400/20' : 'bg-white/[0.04] border-white/10'}`}>
          {isOwnModel ? <Shield className="w-5 h-5 text-emerald-400" /> : <Cpu className="w-5 h-5 text-white/40" />}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <div className={`text-sm font-bold truncate ${isOwnModel ? 'text-emerald-300' : 'text-white/70'}`}>{model.label}</div>
          ...
          <div className="text-xs text-white/30 mt-0.5 truncate">{model.provider} · {model.model_id}</div>
        </div>
        ...
        {!isOwnModel && (
          <div className="text-right shrink-0 ml-2">
            ...
            <div className="text-[10px] text-white/30">vs FairAI</div>
          </div>
        )}
      </div>
      ...
      <div className="px-5 pb-4 flex flex-wrap gap-2">
        <span className="px-2.5 py-1 rounded-lg text-xs font-bold bg-white/[0.04] border border-white/[0.08] text-white/50">
        ...
        <div className="px-5 pb-5 border-t border-white/[0.06] pt-3">
          ...
          {expandedTab === 'radar' && model.radar && (
            <div className="space-y-2.5">
              <div className="text-xs font-bold uppercase tracking-widest text-white/25 mb-3">Radar Breakdown</div>
              {Object.entries(model.radar).map(([k, v]) => (
                <RadarDiff key={k} label={k.replace(/_/g, ' ')} fairai={fairaiScore} other={v} />
              ))}
              {model.reasoning && (
                <p className="text-xs text-white/30 mt-3 pt-3 border-t border-white/[0.06] leading-relaxed italic">
                  "{model.reasoning}"
                </p>
              )}
            </div>
          ...
          {expandedTab === 'graph' && model.skill_matches?.length > 0 && (
            <div className="mt-2 rounded-xl overflow-hidden border border-white/[0.06]" style={{ height: 340 }}>
      ...
      <table className="w-full text-xs">
        ...
        <tbody className="divide-y divide-white/[0.04]">
          {models.map((m, i) => {
            ...
            return (
              <tr key={i} className={`hover:bg-white/[0.02] transition-colors ${isOwn ? 'bg-emerald-400/[0.03]' : ''}`}>
                <td className="px-4 py-3">
                  <div className={`font-bold ${isOwn ? 'text-emerald-300' : 'text-white/60'}`}>{m.label}</div>
                ...
                <td className="text-center px-3 py-3">
                  <span className={`font-black px-2 py-0.5 rounded text-sm ${m.max_delta > 8 ? 'bg-red-400/10 text-red-400' : m.max_delta > 4 ? 'bg-yellow-400/10 text-yellow-400' : 'text-white/40'}`}>
                    {m.max_delta}
                  </span>
                </td>
                <td className="text-center px-3 py-3">
                  <span className={`text-xs px-2 py-0.5 rounded font-bold border ${
                    isOwn ? 'bg-emerald-400/10 text-emerald-400 border-emerald-400/20' : 'bg-white/[0.04] text-white/40 border-white/[0.08]'
...
function AggregateBanner({ data }) {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
      {items.map((it, i) => (
        <div key={i} className={`rounded-xl border p-4 text-center ${it.positive ? 'border-emerald-400/20 bg-emerald-400/[0.04]' : it.highlight ? 'border-red-400/20 bg-red-400/[0.04]' : 'border-white/[0.08] bg-white/[0.02]'}`}>
          <div className={`text-xl font-black ${it.positive ? 'text-emerald-400' : it.highlight ? 'text-red-400' : 'text-white/80'}`}>{it.value}</div>
          <div className="text-xs font-bold text-white/50 mt-0.5">{it.label}</div>
          <div className="text-[10px] text-white/25 mt-0.5">{it.sub}</div>
        </div>
      ))}
    </div>
  );
    ...
    <section className="mb-6 animate-fade-in-up stagger-8">
      ...
      {!data ? (
        <div className="glass-card rounded-2xl p-8 text-center">
          ...
          <p className="text-sm text-white/30 mb-2 max-w-md mx-auto leading-relaxed">
          ...
          <p className="text-xs text-white/20 mb-6 max-w-sm mx-auto">
          ...
          <button
            ...
            className={`px-6 py-3 rounded-xl text-sm font-bold border transition-all hover-lift flex items-center gap-2 mx-auto ${isRunning
              ? 'bg-white/[0.03] text-white/30 border-white/[0.06] cursor-wait'
        ...
        <div className="glass-card rounded-2xl p-10 flex flex-col items-center gap-4">
          ...
          <p className="text-sm text-white/40 text-center">Scoring across {data?.models?.length ?? 4} models with demographic mutations…</p>
        </div>
      ) : (
        <div className="glass-card rounded-2xl overflow-hidden">
          ...
          <div className="px-5 py-4 border-b border-white/[0.06]">
            <div className="flex items-center gap-3 flex-wrap mb-2">
              ...
              <span className="text-xs text-white/30 ml-auto">σ = {data.cross_model_variance} across models</span>
            </div>
            ...
            <div className="flex gap-1 mb-5 bg-white/[0.03] rounded-xl p-1 border border-white/[0.06]">
              {[{ id: 'cards', label: 'Model Cards' }, { id: 'table', label: 'Bias Matrix' }].map(t => (
                <button key={t.id} onClick={() => setActiveTab(t.id)}
                  className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${activeTab === t.id ? 'bg-white/[0.08] text-white' : 'text-white/30 hover:text-white/50'}`}>
                  {t.label}
                </button>
              ))}
            </div>
